...
export default function HistoryScreen({ users }: { users: { name: string }[] }) {
  ...
  // Список имён для фильтра: пользователи + встречающиеся в журнале (напр. «Система», «Клиент»).
  ...
  const preset = (label: string, f: string, t: string) => (
    <button onClick={() => { setFrom(f); setTo(t) }} style={{ padding: '6px 12px', borderRadius: 20, border: 'none', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', background: (from === f && to === t) ? COLORS.primary : '#f1efec', color: (from === f && to === t) ? '#fff' : '#6b655b' }}>{label}</button>
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 80px)', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginBottom: 14, flexShrink: 0 }}>
        <select style={{ ...INP, minWidth: 200 }} value={user} onChange={e => setUser(e.target.value)}>
        ...
        {preset('Месяц', shiftStr(30), todayStr())}
