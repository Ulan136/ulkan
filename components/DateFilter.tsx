...
// Попадает ли дата в выбранный период/конкретный день. day (YYYY-MM-DD) имеет
// приоритет над period. Пустой period='all' и пустой day → показываем всё.
...
// Компактная панель фильтра по дате просмотра. day перекрывает period.
export default function DateFilter({ period, day, onChange }: {
  period: Period
  day: string
  onChange: (period: Period, day: string) => void
}) {
  const chipStyle = (on: boolean): CSSProperties => ({
    padding: '6px 12px', borderRadius: 20, border: 'none', fontSize: 12.5, fontWeight: on ? 700 : 500,
    cursor: 'pointer', fontFamily: 'inherit', background: on ? PRIMARY : '#f1efec', color: on ? '#fff' : '#6b655b',
  })
  ...
  return (
    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', marginBottom: 14 }}>
      ...
      <input type="date" value={day} onChange={e => onChange(period, e.target.value)}
        style={{ padding: '5px 8px', borderRadius: 8, border: `1.5px solid ${day ? PRIMARY : '#e6e2dc'}`, fontSize: 12.5, fontFamily: 'inherit', color: '#26231f', outline: 'none' }} />
