      ...
      <div style={{ maxWidth: 920, margin: '0 auto', padding: '32px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: '#5f5952' }}>Отслеживание заказа</div>
          </div>
        ...
        {showLogin && (
          <div style={{ background: '#fff', borderRadius: 12, padding: 20, marginBottom: 20, boxShadow: '0 0 0 1px #e6e2dc' }}>
            ...
            {loginErr && <div style={{ color: '#b03020', fontSize: 14, marginTop: 8 }}>{loginErr}</div>}
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 4, background: '#e6e2dc', borderRadius: 10, padding: 4, marginBottom: 24, width: 'fit-content' }}>
          {(['track', 'submit'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ padding: '8px 20px', borderRadius: 7, border: 'none', fontWeight: 600, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit', background: tab === t ? '#fff' : 'transparent', color: tab === t ? '#211f1c' : '#5f5952', boxShadow: tab === t ? '0 1px 4px rgba(0,0,0,.1)' : 'none' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div>
                        ...
                        <div style={{ color: '#5f5952', fontSize: 13, marginTop: 2 }}>обновлено {fmtTime(new Date().toISOString())}</div>
                      </div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 28, fontWeight: 700, color: barColor(trackData.progress) }}>{trackData.progress}%</div>
                      {trackData.legStage && (
                        <div style={{ marginTop: 6, display: 'inline-block', fontSize: 12, fontWeight: 600, padding: '2px 10px', borderRadius: 20, ...legStageStyle(trackData.legStage) }}>
                          {trackData.legStage}
                        </div>
                      )}
                    </div>
                    ...
                    {STEPS.map((step, i) => {
                      ...
                      return (
                        <div key={step} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
                          ...
                          <div style={{ width: 26, height: 26, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1, fontSize: 12, fontWeight: 700, background: done ? '#3a9d6e' : current ? '#d4613a' : '#e6e2dc', color: done || current ? '#fff' : '#5f5952' }}>
                            {done ? '✓' : i + 1}
                          </div>
                          <div style={{ fontSize: 12, color: done ? '#2e8a5e' : current ? '#d4613a' : '#5f5952', marginTop: 6, fontWeight: current ? 700 : 400, textAlign: 'center' }}>{step}</div>
                        </div>
                      )
                    })}
                  </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 0 0 1px #e6e2dc' }}>
                      ...
                      {trackData.positions.length === 0
                        ? <div style={{ color: '#5f5952', fontSize: 14 }}>Позиции не сформированы</div>
                        : trackData.positions.map((p, i) => (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: i < trackData.positions.length - 1 ? '1px solid #f1efec' : 'none' }}>
                            <div>
                              <div style={{ fontSize: 14, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 6 }}><RalDot code={extractRal(p.name)} size={13} />{p.name}</div>
                              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: '#6b645b' }}>{p.id}</div>
                              <div style={{ fontSize: 13, color: '#5f5952' }}>{p.qty} {p.unit}</div>
                            </div>
                    ...
                    {trackData.status !== 'Доставлено' && trackData.status !== 'Архив' && (
                      <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 0 0 1px #e6e2dc' }}>
                        ...
                        {changeSent
                          ? <div style={{ color: '#2e8a5e', fontSize: 14, padding: '10px 0' }}>✓ Изменение отправлено менеджеру</div>
                          : (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    <div style={{ background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 0 0 1px #e6e2dc' }}>
                      ...
                      {trackData.details.map((d, i) => (
                        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: i < trackData.details.length - 1 ? '1px solid #f1efec' : 'none' }}>
                          <span style={{ fontSize: 13, color: '#5f5952' }}>{d.k}</span>
                          <span style={{ fontSize: 14, fontWeight: 500 }}>{d.v}</span>
                        </div>
                      ))}
                    </div>
                      ...
                      {trackData.history.slice(0, 6).map((h, i) => (
                        <div key={i} style={{ display: 'flex', gap: 10, padding: '6px 0', borderBottom: i < Math.min(trackData.history.length, 6) - 1 ? '1px solid #f1efec' : 'none', alignItems: 'flex-start' }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: 12, color: '#5f5952' }}>{fmtTime(h.time)}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div style={{ background: '#fff0ea', borderRadius: 12, padding: 16, boxShadow: '0 0 0 1px #f3d5c6' }}>
                      <div style={{ fontSize: 13, color: '#5f5952', marginBottom: 4 }}>Вопросы? Звоните:</div>
          ...
          <div className="anim-fade">
            {subResult ? (
              <div style={{ background: '#fff', borderRadius: 14, padding: 32, boxShadow: '0 0 0 1px #e6e2dc', textAlign: 'center', maxWidth: 480, margin: '0 auto' }}>
                ...
                <p style={{ color: '#5f5952', fontSize: 14, marginBottom: 20 }}>Сохраните ссылки для отслеживания</p>

                {[{ label: 'Кабинет', url: subResult.clientUrl, key: 'cab' }, { label: 'Трекинг', url: subResult.trackingUrl, key: 'trk' }].map(({ label, url, key }) => (
                  <div key={key} style={{ marginBottom: 10 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#5f5952', marginBottom: 4, textAlign: 'left' }}>{label.toUpperCase()}</div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <span style={{ flex: 1, background: '#f1efec', borderRadius: 7, padding: '8px 12px', fontSize: 13, wordBreak: 'break-all', textAlign: 'left' }}>{url}</span>
              ...
              <div style={{ maxWidth: 480, margin: '0 auto', background: '#fff', borderRadius: 14, padding: 28, boxShadow: '0 0 0 1px #e6e2dc' }}>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div>
                    <label style={{ fontSize: 13, fontWeight: 600, color: '#5f5952', marginBottom: 4, display: 'block' }}>ФИО / КОМПАНИЯ *</label>
                    <input style={inp} value={subName} onChange={e => setSubName(e.target.value)} placeholder="Нипа Алматы" required />
                  </div>
                  <div>
                    <label style={{ fontSize: 13, fontWeight: 600, color: '#5f5952', marginBottom: 4, display: 'block' }}>ТЕЛЕФОН *</label>
                    <input style={inp} value={subPhone} onChange={e => setSubPhone(e.target.value)} placeholder="+7 700 000 00 00" required />
                  </div>
                  <div>
                    <label style={{ fontSize: 13, fontWeight: 600, color: '#5f5952', marginBottom: 4, display: 'block' }}>ТЕКСТ ЗАЯВКИ *</label>
