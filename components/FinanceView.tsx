    ...
    <div className="anim-fade">
      ...
      {loading ? (
        ...
        <>
          ...
          {balance !== 0 && (
            <div style={{ fontSize: 13, color: '#5f5952', textAlign: 'center', marginBottom: 16 }}>
          ...
          {!data?.configured && (
            <div style={{ background: '#fdf8e1', border: '1.5px solid #f0d98a', borderRadius: 12, padding: '14px 16px', marginBottom: 16, fontSize: 14, color: '#8a6f00' }}>
          ...
          {(!data || data.transactions.length === 0) ? (
            <div style={{ background: '#fff', borderRadius: 12, padding: 28, textAlign: 'center', color: '#5f5952', fontSize: 14, boxShadow: '0 0 0 1px #e6e2dc' }}>
              Пока нет операций
            </div>
          ) : (
              ...
              {data.transactions.map(t => (
                <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#fff', borderRadius: 10, padding: '10px 14px', boxShadow: '0 0 0 1px #e6e2dc' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 600 }}>{t.type === 'payment' ? 'Оплата' : 'Начисление'}</div>
                    <div style={{ fontSize: 12, color: '#5f5952' }}>{fmtDate(t.date)}{t.comment ? ` · ${t.comment}` : ''}</div>
                  </div>
